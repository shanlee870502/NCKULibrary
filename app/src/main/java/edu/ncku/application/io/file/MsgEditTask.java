...
public class MsgEditTask extends AsyncTask<List<Integer>, Void, Void> implements IOConstatnt{

    private static final String DEBUG_FLAG = MsgEditTask.class.getName();
    private static final String SUB_FILE_NAME = ".messages";

    private Context context;
    private String fileName;
    private List<Boolean> msgRead;

    public MsgEditTask(Context context, List<Boolean> msgRead) {
        ...
        try {
            ...
            if (!msgFile.exists()) {
                ...
                for(int index : params[0]){
                    readMsgs.get(index).setIsRead(msgRead.get(counter));
                    if(showLogMsg){
                        Log.d(DEBUG_FLAG, "update " + index + " from sd");
                    }
                    counter++;
                }
